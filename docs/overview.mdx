...
- **ONS Postcode Directory (ONSPD)** — UK postcodes with administrative, electoral, statistical and health geographies. Updated quarterly (February, May, August, November).
- **OS Open Names** — places, settlements and other gazetteer entries for Great Britain. Updated quarterly.
- **Scottish Postcode Directory (SPD)** — postcodes with Scottish-specific geographies (council areas, Scottish Parliamentary constituencies, Data Zones, SIMD, etc.). Updated twice a year.

For the exact release date of each dataset currently loaded into the public API and Docker images, see the [CHANGELOG](https://github.com/ideal-postcodes/postcodes.io/blob/master/CHANGELOG.md) on GitHub.
