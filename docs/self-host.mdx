...
`idealpostcodes/postcodes.io.db` is built on top of the official `postgis/postgis` image and ships a pre-loaded `pg_dump` of the latest ONSPD / OS Open Names / Scottish Postcode Directory data, denormalised into the `pcio.*` schema (`pcio.onspd`, `pcio.opennames`, `pcio.spd`). The image is rebuilt and re-tagged each time the upstream dump is refreshed; see the [Changelog](https://github.com/ideal-postcodes/postcodes.io/blob/master/CHANGELOG.md) for dataset cadence.
